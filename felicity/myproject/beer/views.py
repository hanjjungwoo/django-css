...
#             tmp = pd.DataFrame(data=[tmp], columns=['아이디', '도시', '평점'])
#             ratings = pd.concat([ratings, tmp])

#         uname = name
#         ratings_matrix = ratings.pivot_table('평점', index='아이디', columns='도시')
#         ratings_matrix = ratings_matrix.fillna(0)
#         ratings_matrix_T = ratings_matrix.transpose()
...
#             ratings_matrix.values, item_sim_df.values, n=5)
...
#         recomm_beer = recomm_beer_by_userid(
#             ratings_pred_matrix, uname, not_tried, top_n=3)
...
#             target = target[['Aroma', 'Appearance',
...
#             try:
#                 category.append(
#                     beer_data[beer_data['도시이름'] == result[i]]['Main Category'].values[0])
#                 food.append(beer_data[beer_data['도시이름'] ==
#                             result[i]]['Paring Food'].values[0])
#             except:
#                 category.append('수집되지 않았습니다.')
#                 food.append('수집되지 않았습니다.')
...
#         tmp_ratings = []
#         for i in range(3):
#             target = beer_year[beer_year['도시'] == result[i]]
#             target_year = target['년'].tolist()
#             target_rating = target['평점'].tolist()
#             tmp_year.append(target_year)
#             tmp_ratings.append(target_rating)
